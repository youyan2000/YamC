...
| 方式 | 入口 | 适合 |
|------|------|------|
| **GUI** | `python yamc/yaml_config_builder.py` | 想点按钮、看反馈、在工程目录内做骨架/调参 |
| **CLI 流水线** | `python yamc/yamc_cfg.py -d <工程> --topology buck` | 一键把 HardC 接入**真实 CubeMX 工程**（含三宏注入 + 编译） |
...
yamc 处理两种项目形态，先分清，后面的教程才不会走错目录：

- **仓库内骨架工程（HardC 根就是工程）**：用 GUI Tab2，产出在 `build/gen/<name>/`，不碰外部 CubeMX。适合练手 / 快速看拓扑生成。
- **真实 CubeMX 工程（外部，`*.ioc`）**：用 CLI `yamc_cfg.py`（或 GUI「外部工程接入」），把 HardC 以 submodule 接进去、注入 CMake + 三宏、编译真实固件。**这是最终交付形态。**
...
```bash
python yamc/yaml_config_builder.py
```
...
- 填/浏览工程根（含 `.ioc`）→ 「探测」确认平台 → 勾 skip 选项 → 「▶ 运行完整接入」。这个按钮跑的就是 `yamc_cfg` 的**同一条流水线**（engine 后台线程），输出进日志。
...
| 按钮 | 等价 CLI |
|------|----------|
| scaffold scan | `python yamc/scaffold.py scan` |
| scaffold gen | `python yamc/scaffold.py gen <Config/projects/xxx.yaml>`（逐个跑当前工程） |
| scaffold deps <module_id> | `python yamc/scaffold.py deps <module_id>` |
| flash_map list / show / gen | `python yamc/flash_map_gen.py list / show / gen` |
| merge selftest / merge / info | `python yamc/merge_firmware.py selftest / merge … / info <hex>` |
...
python yamc/scaffold.py scan

# 查看某模块的传递闭包依赖（拓扑排序 BSP→…→App）
python yamc/scaffold.py deps components/pid
python yamc/scaffold.py deps module/power

# 从工程配置生成骨架 → build/gen/<project>/
python yamc/scaffold.py gen Config/projects/buck.yaml
python yamc/scaffold.py gen Config/projects/buck.yaml --out build/gen/my_buck
```
...
```bash
python yamc/flash_map_gen.py list                      # 列出 flash_map.yaml 全部 MCU 与分区概况
python yamc/flash_map_gen.py show --mcu stm32f334     # 展示某 MCU 完整解析分区
python yamc/flash_map_gen.py gen --mcu stm32f334      # 生成 bsp_flash_map.h + bootloader/app 两个 .ld
python yamc/flash_map_gen.py gen --out build/gen/x     # 自定输出目录
```
...
python yamc/merge_firmware.py merge bootloader.hex app.hex -o merged.hex

# 查看单个 hex 的地址范围 / 段
python yamc/merge_firmware.py info build/gen/buck/build/app.hex
...
```bash
python yamc/yamc_ioc.py -d <工程根>                     # 摘要 + 写 .hardc/<stem>.periph.yaml
python yamc/yamc_ioc.py -d <工程根> -o out.yaml        # 自定输出路径
python yamc/yamc_ioc.py -d <工程根> --verbose           # 详细日志
python yamc/yamc_ioc.py -d <工程根> --force             # 忽略缓存强制重解析
```

从 CubeMX `.ioc` 提取 MCU/HRTIM/ADC/UART/CAN 外设信息 → YAML + 控制台摘要（MCU 型号、外设计数）。等价 `xr_parse_ioc`，供 `yamc_cfg`/GUI 复用同一解析逻辑。

### 3.5 与 LibXR 工具链指令对照（对齐基准：`LibXR_CppCodeGenerator`）

| LibXR 指令 | 作用 | HardC 对应 | 状态 |
|-----------|------|-----------|------|
| `xr_cubemx_cfg` | 配置 CubeMX 工程 + submodule + 生成 | `yamc_cfg.py` | ✅ 对标（含三宏注入/编译）|
| `xr_parse_ioc` | `.ioc` → YAML + 摘要 | `yamc_ioc.py` | ✅ 新增 |
...
```bash
python yamc/yamc_cfg.py -d <工程根> --topology buck
```
...
4. 往工程 `CMakeLists.txt` 注入一个幂等 `yamc HardC BEGIN/END` 块：
...
python <HardC>/yamc/yamc_cfg.py -d . --topology buck --git-source https://github.com/youyan2000/HardC.git
# 成功标志（日志）：
#   ✓ 生成 app_main.c/h
...
| 接入已有 CubeMX 工程 | CLI `yamc_cfg.py -d . --topology …` | 最常用 |
...
| 症状 | 原因 / 处理 |
|------|------------|
| 编译报 `nmake: no such file or directory` | cmake 用了默认 NMake 生成器。用 `-G Ninja`（yamc 现默认 Ninja）；确认本机装 Ninja |
