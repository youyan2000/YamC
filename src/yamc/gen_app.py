...
def topo_to_power_cfg(topo: dict, params: Optional[dict] = None) -> dict:
    ...
    roles → adc_ch_* 映射按 control_module 形态 (supercap 特例):
      gen_supercap: 相电流角色 i0/i1/i2 → adc_ch_i[] 数组 (GenSuperCapCfg 字段);
    ...
    if cm == "gen_supercap":
        ...
        # 非槽位保护/滤波默认 (gen_supercap.h 注释: 短路 2×i_lim_a, 失平衡 0.5×i_lim_a, LPF 120Hz).
        ...
        # gen_vsi 用 PwmSvpwm / AdcAcSampler 实例 (非 cfg 通道字段), 不映射 PWM/ADC 平铺.
        # 非槽位控制/保护默认缺省由 gen_vsi_init 内部派生, 无需显式发射.
        pass
    else:
...
_SUPERCAP_C_TEMPLATE = """// ==== 生成: 拓扑=__TOPOLOGY__ (契约 §4.8; CAN 订阅分发: com_can 订阅表 → hmi_can_on_frame) ====
// 事件日志接缝 (app 侧提供; 发射块内自足声明 — 模板未含 extern, 见 AGENT-SYNC)
extern void log_evt(uint8_t ev);
// CAN I/O 接缝 — 生产订阅分发: RX 经 can_poll(&g_root.drv_can) 分发订阅表 → can_rx_061 → hmi_can_proto 解析
//   hmi_can_bind(send=can_send_tx, poll=NULL, on_referee) — RX 由 com_can 订阅表驱动, 不再轮询 hmi_can_poll
...
static void can_on_referee(const HmiCanReferee *ref) {
  // 0x061 有效帧 → 超电功率指令 (命令邮箱, MAIN→FAST 周期边界生效)
  if (ref->enable_conv)
    gen_supercap_set_referee_power(&g_root.supercap, ref->power_limit_w);
}
static void can_rx_061(Can *me, const CanFrame *frame, void *ctx) {
  (void) me;
  (void) ctx;
  hmi_can_on_frame(&g_root.can_proto, frame->id, frame->data, frame->dlc);
}

static void modules_apply_config(void) {
  // 均流 cfg 同步 + 主模块 cfg 整体拷贝 (PowerCfg == GenSuperCapCfg, 零漂移)
  g_root.share.cfg = g_cfg.share;
  g_root.supercap.cfg = g_cfg.power;
  // 派生: 功率环 PI 限幅/迟滞窗口 + share_gain 注入均流模块
  gen_supercap_sync_cfg(&g_root.supercap);
}

static void modules_board_init(void) {
  // 均流 + 主模块实例化 (cfg 来自 g_cfg, init 内部已 sync)
  gen_share_init(&g_root.share, &g_cfg.share);
  gen_supercap_init(&g_root.supercap, &g_cfg.power);
  // 致命绑定序: 先 share→pwm, 再 supercap→adc+share (base.pwm 快照; 反序急停不封波, gen_supercap.h 头注释)
  gen_share_bind(&g_root.share, &g_root.drv_buck_pwm);
  gen_supercap_bind(&g_root.supercap, &g_root.drv_dc_adc, &g_root.share);
  // CAN 协议 (ctx main): 订阅分发 — send/on_referee 接缝 + 0x061 订阅 (com_can 表满 → ERR_FULL, 启动期即暴露)
  hmi_can_init(&g_root.can_proto);
  hmi_can_bind(&g_root.can_proto, can_send_tx, NULL, can_on_referee);
  (void) can_register(&g_root.drv_can, HMI_CAN_RX_ID, can_rx_061, &g_root.can_proto);
...
static void modules_fast_tick(void) {
  ...
  gen_supercap_tick(&g_root.supercap);
}
...
static void modules_slow_tick(void) {
  ...
  // 1kHz SLOW ÷5 = 200Hz 遥测发送 (hmi_can_tx_telemetry → can_send_tx)
  ...
  if (++can_tx_div >= 5) {
    ...
    hmi_can_tx_telemetry(&g_root.can_proto, &tel);
    can_tx_div = 0;
  }
}

static void modules_main_loop(void) {
  // CAN 轮询 (DMA FIFO → 订阅表分发): 0x061 → can_rx_061 → hmi_can_on_frame → can_on_referee
  can_poll(&g_root.drv_can);
  // 保护事件排空 (SPSC 环, FAST 单生产者; 事件字节 = comp_error.h 位掩码)
  uint8_t ev;
  while (gen_supercap_evt_pop(&g_root.supercap, &ev))
    log_evt(ev);
}
...
def _gen_mod_supercap(topo: dict) -> tuple[str, str, str]:
    ...
    h_cfgstruct = (f"typedef GenSuperCapCfg PowerCfg;  // 拓扑主控制模块 "
    ...
    h_root = (
        ...
        f"GenCurrentShare share;      // 三相均流 (gen_current_share.h)\n"
        f"GenSuperCap supercap;       // 超级电容功率控制 (gen_supercap.h)"
    )
...
_VSI_C_TEMPLATE = """static void modules_apply_config(void) {
  ...
  gen_vsi_sync_cfg(&g_root.vsi);
}

static void modules_board_init(void) {
  // GenVsi 实例化 (cfg 来自 g_cfg.power) + 绑定 SVPWM/AC采样设备
  gen_vsi_init(&g_root.vsi, &g_cfg.power, /*grid_freq_hz=*/__GRID_HZ__);
  gen_vsi_bind(&g_root.vsi, &g_root.drv_svpwm, &g_root.drv_ac_adc);
...
static void modules_main_loop(void) {
  // 保护事件排空 (SPSC 环, FAST 单生产者; 事件字节 = comp_error.h 位掩码)
  uint8_t ev;
  while (gen_vsi_evt_pop(&g_root.vsi, &ev))
    log_evt(ev);
}
...
def _gen_mod_vsi(topo: dict) -> tuple[str, str, str]:
    ...
    h_cfgstruct = (f"typedef GenVsiCfg PowerCfg;  // 拓扑主控制模块 "
                   f"(control_module: {topo.get('control_module')})")
    adc = topo.get("adc") or {}
    h_root = (
        f"// ==== 由 yamc_cfg 生成: 电源域根成员 (拓扑={name}) ====\n"
        ...
        f"// --- Power-OOP: 电源控制 Module ---\n"
        f"GenVsi vsi;             // 三相 VSI 控制 (gen_vsi.h)"
    )
    return c_block, h_cfgstruct, h_root


_MODULE_GEN = {
    "gen_supercap": _gen_mod_supercap,
    "gen_vsi": _gen_mod_vsi,
}
...
def _bootloader_cfg_block(boot_cfg: dict) -> str:
  """bootloader 拓扑段 → boot_loader 默认配置填充 (取 bsp_flash_map.h 宏).
  ...
  lines = [
    "// yamc 注入: boot_loader 默认配置 (app 分区来自 bsp_flash_map.h, 与 app_flash.ld 同源)",
